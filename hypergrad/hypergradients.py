...
def hgvesj(params,
    ...
    # experimental code that uses the inner solver procedure as input
    ...
    us = [torch.randn(hparam.size()) for hparam in hparams]
    us = [u/torch.norm(u, 2) for u in us]
    hparams_mu = [mu*u + hparam for u, hparam in zip(us, hparams)]
    params_mu = inner_solver(hparams_mu, steps=T, params0=params0, optim=optim)

    deltas = [(param_mu - param) * p / mu for param_mu, param in zip(params_mu, params)]
...
def sczoj(params_list,
    ...
    """StoChastic Zeroth-Order Jacobian
    :return:
    """
    ...
    if more:
        ...
        o_loss = outer_loss(params, hparams)
    grad_outer_ws, grad_outer_hparams = get_outer_gradients(o_loss, params, hparams)
...
def hozog(params,
    ...
    # zeroth order method described in Bin Gu et al (2021).
    ...
    if more:
        f, acc = outer_loss(params, hparams, more=more)
    else:
        f = outer_loss(params, hparams)

    f_mu = outer_loss(params_mu, hparams_mu)
    grads = [(f_mu - f) * u * p / mu for u in us]
...
def reverse(params_history: List[List[Tensor]],
    ...
    grad_outer_w, grad_outer_hparams = get_outer_gradients(o_loss, params_history[-1], hparams) # output is (grad wrt last iterate of w, grad wrt hparams)
...
def fixed_point(params: List[Tensor],
    ...
    grad_outer_w, grad_outer_hparams = get_outer_gradients(o_loss, params, hparams) # output is (grad wrt last iterate of w, grad wrt hparams)
